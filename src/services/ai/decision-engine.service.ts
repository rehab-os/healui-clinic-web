...
export class PhysioDecisionEngine {
  ...
  private getDetailedAssessmentSequence(): string[] {
    ...
    assessments.push('posture_assessment', 'gait_analysis', 'adl_scoring');
  ...
  private generateProvisionalDiagnosis(): string[] {
    ...
    if (this.session.activatedPathways.has(PathwayType.PAIN)) {
      ...
      if (this.isPainInLocation(painLocation, ['shoulder'])) {
        if (aggravatingFactors?.includes('overhead')) {
          diagnosis.push('🎯 Shoulder Impingement Syndrome (suspected - requires further testing)');
      ...
      if (this.isPainInLocation(painLocation, ['back', 'spine', 'lumbar'])) {
        if (aggravatingFactors?.includes('bending_forward')) {
          diagnosis.push('🎯 Lumbar Disc Herniation with radiculopathy (suspected - requires further testing)');
      ...
      if (this.isPainInLocation(painLocation, ['knee'])) {
        if (responses['swelling_assessment'] !== 'absent') {
          diagnosis.push('🎯 Knee Joint Inflammation/Effusion');
        } else if (painNature?.includes('sharp') || aggravatingFactors?.includes('stairs')) {
          diagnosis.push('🎯 Meniscal or Ligamentous Pathology (requires further testing)');
        } else {
      ...
      if (this.isPainInLocation(painLocation, ['neck', 'cervical'])) {
        ...
        } else if (responses['sensation_type']?.includes('numbness')) {
          diagnosis.push('🎯 Cervical Radiculopathy (suspected - requires further testing)');
        } else {
  ...
  private generateRecommendations(): string[] {
    ...
    // Pain-based follow-up recommendations
    if (this.session.activatedPathways.has(PathwayType.PAIN) && responses['vas_score'] > 5) {
      recommendations.push('🔬 Further diagnostic testing may be required');
  ...
  filterRelevantConditions(allConditions: any[], clinicalFindings: any): any[] {
    ...
    // 4. SYMPTOM-BASED CONDITION FILTERING
    // Since we removed special tests, use symptom patterns for condition filtering
    if (responses['pain_nature'] || responses['aggravating_factors']) {
      const painNature = responses['pain_nature'] || [];
      const aggravatingFactors = responses['aggravating_factors'] || [];
      
      // Filter by symptom characteristics
      const symptomConditions = allConditions.filter(condition => {
        ...
        if (painNature.includes('sharp') && responses['pain_location']?.includes('knee')) {
      ...
      relevantConditions = [...new Set([...relevantConditions, ...symptomConditions])];
      console.log(`🎯 Symptom-based filter: conditions filtered by symptoms`);
    }
  ...
  async getAIDifferentialDiagnosis(): Promise<any> {
    try {
      ...
      const { aiDiagnosticService } = await import('./diagnostic.service');
