...
const testimonials = [
 {
  ...
  rating: 5,
  text:
   "The AI diagnostic chatbot has transformed our initial assessments. It's like having a clinical expert guiding every evaluation.",
 },
 {
  ...
  rating: 5,
  text:
   "Our treatment efficiency improved by 40% with AI-powered protocols. Patients recover faster with personalized exercise plans.",
 },
 {
  name: "Dr. Anita Verma",
  ...
  rating: 5,
  text:
   "The red flag detection system has prevented critical conditions from being missed. It's a game-changer for patient safety.",
 },
 {
  ...
  rating: 5,
  text:
   "Documentation time reduced by 75%. More time for patients, better outcomes. The ROI has been exceptional.",
 },
];
...
export default function GetStarted() {
 ...
 const handleSendOTP = async (e: React.FormEvent) => {
  ...
  try {
   ...
   const confirmationResult = await firebaseAuthService.sendOTP(
 ...
 const handleVerifyOTP = async (otpValue?: string) => {
  ...
  try {
   ...
   const firebaseIdToken = await firebaseAuthService.verifyOTP(
    confirmationResult,
    otpString,
   );
   ...
   if (!createOrgResponse.success) {
    ...
    if (
     createOrgResponse.message?.includes("already exists") ||
     createOrgResponse.message?.includes("duplicate")
    ) {
     throw new Error(
      "An organization with this name or phone already exists. Please try different details.",
     );
    }
    throw new Error(
     createOrgResponse.message || "Failed to create organization",
    );
   }
   ...
   try {
    ...
    if (loginResponse.success) {
     ...
     console.warn(
      "Organization created but login failed:",
      loginResponse.message,
     );
     router.push(
      "/login?message=Organization created successfully. Please login to continue.",
     );
     return;
    }
 ...
 return (
  <div className='min-h-screen bg-white lg:bg-gradient-to-br lg:from-brand-teal/5 lg:via-white lg:to-brand-teal/5 flex'>
   ...
   <div className='hidden lg:flex lg:w-1/2 bg-gradient-to-br from-brand-teal to-brand-black relative overflow-hidden'>
    ...
    <div className='relative z-10 flex flex-col justify-center p-12 text-white'>
     ...
     <div className='space-y-5'>
      <div className='relative w-28 h-28 mb-2 rounded-2xl bg-white/10 backdrop-blur-sm p-2 overflow-hidden'>
       <Image
        src='/welcome_masscott_two.png'
        alt='Healui Mascot'
        fill
      ...
      <div>
       <h2 className='text-3xl font-bold mb-3'>
        Transform Your Physiotherapy Practice
       </h2>
       <p className='text-white/70 text-base leading-relaxed'>
        Join physiotherapists using AI-powered tools to deliver better patient
        outcomes.
       </p>
      </div>
   ...
   <div className='w-full lg:w-1/2 flex flex-col'>
    ...
    <div className='flex justify-between items-center px-6 py-4 border-b border-gray-100'>
     ...
     <a
      href='/login'
      className='text-brand-teal font-semibold text-sm hover:text-brand-teal/80 transition-colors ml-auto'
     >
      Sign In
     </a>
    </div>
    ...
    {/* Form Content */}
    <div className='flex-1 flex items-start lg:items-center justify-center px-6 pt-0 pb-6 lg:pt-0'>
      ...
      {!otpSent ? (
       ...
       <div className='space-y-5'>
        ...
        <div className='text-center'>
         <div className='relative w-[180px] h-[300px] mx-auto mb-1'>
          <Image
           src='/onboard_masscott_v2.png'
           alt='Healui Mascot'
           fill
         ...
         <div className='inline-flex items-center gap-1.5 bg-brand-teal/10 text-brand-teal text-xs font-medium px-3 py-1 rounded-full mb-3'>
          <Sparkles className='w-3.5 h-3.5' />
          Free forever
         </div>
         <h2 className='text-2xl sm:text-3xl font-bold text-gray-900 mb-2'>
          Your clinic, powered by{" "}
          <span className='bg-gradient-to-r from-brand-teal to-teal-600 bg-clip-text text-transparent'>
           AI
          </span>
         </h2>
        ...
        <form onSubmit={handleSendOTP} className='space-y-4'>
         <div className='space-y-3'>
          <div className='relative'>
           <Building2 className='absolute left-3.5 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400' />
           <input
            type='text'
            id='orgName'
            value={formData.name}
            onChange={(e) => handleInputChange("name", e.target.value)}
            className='w-full pl-11 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-teal focus:border-transparent transition-all text-sm'
         ...
         <button
          type='submit'
          disabled={loading}
          className='w-full bg-brand-teal hover:bg-brand-teal/90 text-white py-3.5 rounded-xl font-semibold text-base transition-all duration-200 disabled:opacity-60 flex items-center justify-center shadow-sm'
         >
          {loading ? (
           <>
         ...
         <div className='flex justify-center gap-2'>
          {otp.map((digit, index) => (
           <input
            ...
            className='w-11 h-11 text-center text-lg font-bold border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-teal focus:border-brand-teal transition-all'
