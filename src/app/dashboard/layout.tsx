...
// Detail/focus pages where sidebar should be hidden
const FULL_SCREEN_PATTERNS = [
  /^\/dashboard\/clinics\/[^/]+$/,        // /dashboard/clinics/[id]
  /^\/dashboard\/patients\/[^/]+$/,       // /dashboard/patients/[id]
  /^\/dashboard\/appointments\/[^/]+\/.+/, // /dashboard/appointments/[id]/[visitId]
  /^\/dashboard\/video-call\/.+/,          // /dashboard/video-call/[visitId]
];
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
  ...
  const hideSidebar = FULL_SCREEN_PATTERNS.some(pattern => pattern.test(pathname));
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="flex h-[calc(100vh-60px)] relative">
        {/* Sidebar — hidden on detail/focus pages */}
        {!hideSidebar && (
          <>
            <aside className={`
              fixed lg:static top-0 lg:top-0 bottom-0 left-0 z-40 bg-brand-white border-r border-gray-200 transform transition-all duration-300 ease-in-out lg:translate-x-0 shadow-lg lg:shadow-sm h-screen lg:h-full
