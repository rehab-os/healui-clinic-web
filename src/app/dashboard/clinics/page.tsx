        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4 flex-1">
            <div className={`h-12 w-12 rounded-full flex items-center justify-center ${
              clinic.is_active ? 'bg-[#c8eaeb]' : 'bg-gray-100'
            }`}>
              <Building2 className={`h-6 w-6 ${
                clinic.is_active ? 'text-[#1e5f79]' : 'text-gray-400'
              }`} />
            </div>
            ...
            {clinic.facilities && clinic.facilities.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {clinic.facilities.slice(0, 3).map((facility, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-[#eff8ff] text-[#1e5f79]"
                  >
                    {facility}
                  </span>
                ))}
