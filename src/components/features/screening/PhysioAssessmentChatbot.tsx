...
import { MessageCircle, Send, Brain, AlertTriangle, Calendar, Mic, Keyboard } from 'lucide-react';
import { PhysioDecisionEngine, QuestionTemplate, AssessmentSession } from '@/services/ai/decision-engine.service';
...
import CustomAssessmentSelector from '../assessments/CustomAssessmentSelector';
...
import { ChatbotAssessmentBuilder } from '@/lib/utils/chatbot';
...
const PhysioAssessmentChatbot: React.FC<PhysioAssessmentChatbotProps> = ({
  ...
  const handleVoiceInputComplete = useCallback((transcription: string) => {
    ...
    // For text questions, set the transcription directly
    if (currentQuestion.type === 'text') {
      setCurrentResponse(transcription);
      return;
    }

    // For choice questions, try to parse the response
    if (currentQuestion.type === 'single_choice' && currentQuestion.options) {
      ...
      if (parsed.value && parsed.confidence > 0.5) {
        const matchedOption = currentQuestion.options.find(
          o => o.label.toLowerCase() === parsed.value.toLowerCase()
        );
        if (matchedOption) {
          setCurrentResponse(matchedOption.value);
          return;
        }
      }
    }
    ...
    if (currentQuestion.type === 'yes_no') {
      ...
      if (lower.includes('yes') || lower.includes('yeah') || lower.includes('yep') || lower.includes('correct')) {
    ...
    if (fullSession?.responses && typeof fullSession.responses === 'object') {
      const additionalData = Object.entries(fullSession.responses)
        .filter(([key, value]) => 
          value && 
          value !== 'not an' && 
          !['chief_complaint', 'vas_score', 'pain_location', 'pain_nature', 'pain_timing', 'swelling', 'tenderness', 'activities_affected', 'work_tasks', 'stair_climbing'].includes(key)
        );
  ...
  const formatTechnicalSummary = (summary: any): string => {
    ...
    // Key Findings
    if (responses.swelling_assessment || responses.gait_analysis || responses.posture_assessment) {
  ...
  const renderQuestionInput = () => {
    ...
    // Voice input toggle for applicable question types
    const showVoiceToggle = ['text', 'single_choice', 'multi_choice', 'scale', 'yes_no'].includes(currentQuestion.type || '');

    const VoiceInputSection = showVoiceToggle ? (
      <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          {isVoiceMode && (
            <span className="text-xs text-gray-500">Tap the microphone to speak</span>
          )}
        </div>
      </div>
    ) : null;
              ...
              <div className="flex flex-col items-center py-6">
                ...
                {currentResponse && (
                  <div className="mt-4 w-full p-3 bg-green-50 border border-green-200 rounded-xl">
                    <p className="text-sm text-green-800 font-medium">Transcribed:</p>
          ...
          <div className="space-y-2">
            ...
            {isVoiceMode ? (
              <div className="flex flex-col items-center py-4">
                ...
                />
                <p className="text-xs text-gray-500 mt-2 text-center">
                  Say the options you want to select (e.g., "pain and swelling")
                </p>
                {Array.isArray(currentResponse) && currentResponse.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2 justify-center">
                    {currentResponse.map((val: string) => {
                      ...
                      return (
                        <span key={val} className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm">
                          {option?.label || val}
                        </span>
                      );
                    })}
          ...
          <div className="space-y-4">
            ...
            {isVoiceMode ? (
              <div className="flex flex-col items-center py-4">
                ...
                />
                <p className="text-xs text-gray-500 mt-2 text-center">
                  Say a number from {currentQuestion.min || 0} to {currentQuestion.max || 10}
                </p>
                {currentResponse !== undefined && currentResponse !== '' && (
                  <div className="mt-3 text-center">
                    <span className="text-3xl font-bold text-blue-600">{currentResponse}</span>
                    <span className="text-gray-500 ml-1">/ {currentQuestion.max || 10}</span>
                  </div>
                )}
              </div>
            ) : (
