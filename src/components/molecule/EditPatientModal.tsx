...
const EditPatientModal: React.FC<EditPatientModalProps> = ({ patient, onClose, onSuccess }) => {
  ...
  // Helper function to parse address from patient data
  const parsePatientAddress = (address?: string | AddressData): AddressData => {
    if (!address) return { country: 'India' };
  ...
  const [addressData, setAddressData] = useState<AddressData>(parsePatientAddress(patient.address));
