...
const ContextSwitcher: React.FC = () => {
  ...
  // Check if current page is authorized for organization context
  const isPageAuthorizedForOrganization = (path: string) => {
    // Pages that are NOT allowed in organization context
    const restrictedPagesInOrgContext = [
      '/dashboard/patients',           // Only for clinic context with specific roles
      '/dashboard/appointments/',      // Most appointment pages need clinic context
      '/dashboard/video-call/',        // Video calls are clinic-specific
    ];
    
    // Pages that ARE allowed in organization context
    const allowedPagesInOrgContext = [
      '/dashboard',                    // Main dashboard
      '/dashboard/team',               // Team management (for owners)
      '/dashboard/clinics',            // Clinic management (for owners)
      '/dashboard/profile',            // Profile settings
      '/dashboard/settings',           // General settings
      '/dashboard/availability',       // Availability settings
    ];
    
    // If it's explicitly allowed, return true
    if (allowedPagesInOrgContext.some(allowedPath => path === allowedPath || path.startsWith(allowedPath))) {
      return true;
    }
    
    // If it's explicitly restricted, return false
    if (restrictedPagesInOrgContext.some(restrictedPath => path.includes(restrictedPath))) {
      return false;
    }
    
    // Default to allowed for unknown pages
    return true;
  };
  ...
  const handleOrganizationChange = () => {
    ...
    // Check if current page is authorized for organization context
    if (!isPageAuthorizedForOrganization(pathname)) {
      // Redirect to dashboard if current page is not authorized
  ...
  const handleClinicChange = (clinic: typeof currentClinic) => {
    if (clinic) {
      ...
      // If switching to clinic context from my-practice and user doesn't have right permissions
      // for current page, redirect to dashboard
      if (currentContext === 'my-practice') {
        const hasPatientAccess = clinic.is_admin || 
                                clinic.role === 'receptionist' || 
                                clinic.role === 'manager';
        
        if (pathname.includes('/dashboard/patients') && !hasPatientAccess) {
          router.push('/dashboard');
        }
      }
  ...
  const handleMyPracticeSelect = () => {
    ...
    // Check if current page works with my-practice context
    if (pathname.includes('/dashboard/patients') || pathname.includes('/dashboard/team') || pathname.includes('/dashboard/clinics')) {
      // Redirect to dashboard for pages that don't work with my-practice
      router.push('/dashboard');
    }
